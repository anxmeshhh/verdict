...
| A pipeline that ingests, cleans, analyzes, models, visualizes | 🔶 ingest+analyze built and measured (Verdict Core + Phase 6 security scenarios, ~75-80% real recall); model+visualize is Phase 7/11 | [`hackathon/security_gen_eval/`](./hackathon/security_gen_eval/) |
...
**Phase 6 (security-shaped scenario generation) — in progress, second phase touched:**

- ✅ **Scenario schema + prompt extended** — [`verdict/generator.py`](./verdict/generator.py): `Scenario` now carries an optional `vuln_class` (`injection` / `auth_bypass` / `secret_leak` / `insecure_deserialization`), and the one bounded LLM call now also proposes security-shaped scenarios where they plausibly apply, alongside the existing correctness scenarios - still one LLM step, not a second call. Cache version bumped so old cached scenarios can't collide with the new schema.
- ✅ **Security-specific hallucination guards** — [`verdict/validator.py`](./verdict/validator.py): four new claim/evidence pattern pairs (matching the existing type-enforcement/logging/thread-safety/format-validation guards), each verified against a real true-positive and true-negative case before being trusted, e.g. a scenario claiming "prevents SQL injection" is rejected unless the diff actually contains a parameterized-query construct.
- ✅ **Deterministic dependency-CVE checker written and live-verified** — [`verdict/depcheck.py`](./verdict/depcheck.py): no LLM at all - parses `requirements.txt`/`pyproject.toml`/`package.json` for exact-pinned versions only (a version range is never guessed at) and looks each one up against real OSV.dev advisories. Verified against a real old `requests==2.6.0` pin: correctly found 14 real, live advisories, not a mocked response.
- ✅ **Finding extraction wired into the pipeline** — [`verdict/findings.py`](./verdict/findings.py): a security-tagged scenario only becomes a Finding if it actually failed the sandbox *after* the confirm-FAILED pass (so a flaky generated test can't produce a false finding), written to `.verdict/findings/<run_id>.json` - the local schema Phase 7's BigQuery table ingests from later. All modules import and wire together cleanly (verified).
- ✅ **Real precision/recall gate run — [`hackathon/security_gen_eval/`](./hackathon/security_gen_eval/)**, against Groq (`openai/gpt-oss-120b`), same scope as Phase 0's original methodology: 20 real CVEfixes commits (5 per vuln_class), through the real scenario-gen call + real validator, no result cherry-picked or hidden.
  - **First run: 80% recall (16/20).** Direct inspection of the 4 misses found two real, fixable issues, not just "the model missed it": (1) a real fix using Perl's `->Quote()` escaping call wasn't recognized by the injection-prevention validator guard, which only knew Python-style `.escape()`/`shlex.quote` — fixed by broadening the evidence pattern; (2) one `secret_leak`-labeled commit's actual issue was an uninitialized memory buffer being logged (CWE-532), not a credential leak — CWE-532 turned out to be a broader real-world category than this project's narrowly-scoped `secret_leak` vuln_class, so the CWE mapping was corrected (CWE-798 + CWE-312 only), not the code.
  - **Re-run after both fixes: 75% recall (15/20)**, with `injection` confirmed fixed (4/5 → 5/5). The small overall dip is *not* a regression — `run_eval.py` deliberately calls `generate(force=True)`, bypassing the scenario-gen cache so every run tests live LLM behavior, and at only 5 examples per class, one example's outcome flipping (LLM output isn't bit-exact even at low temperature) swings a class's recall by 20 points. Both runs are preserved (`gate_results_run1_before_fixes.json` and `gate_results.json`) rather than only keeping the more flattering number.
  - **Bottom line reported honestly:** roughly 75-80% recall on real, known vulnerabilities, with two concrete, evidence-backed fixes already landed and the remaining misses being genuine LLM/classification limits (a wrong-class tag, one transient API error) rather than validator bugs.

**Everything else (Phases 7, 9, 10, 11, 12):** not started. Still exactly what's described in the build plan above, nothing built.

One sequencing note worth being explicit about: **Phase 10 (the four-agent autonomous layer) can't be built before Phase 7**, not because of ordering preference but because the agents' whole job is querying the BigQuery `vulnerability_map` - there's nothing for them to act on until that table exists. Building the agents against fake data first would be exactly the kind of shortcut this project doesn't take. When Phase 10 does land, live status will be shown two ways, not one: a **terminal** live feed (extending the CLI's existing staged live-progress pattern, already real and working for a single check today) for local dev/demo use, and the **Looker dashboard** (Phase 11) as the always-on passive view for when nobody's watching a terminal.
