...
from verdict.reporter import build_record, format_json, load_run, new_run_id, save_run
...
def run(
    ...
    if needs_llm and not status.reachable:
        _fail("config", f"Ollama not reachable at {config.ollama_url}")
    if needs_llm and config.model not in status.models:
        _fail("config", f"model '{config.model}' not pulled")
    if not docker_ok:
        _fail("config", "Docker daemon not reachable")
    ...
    if not intent_result.diff.strip():
        _fail("intent", "diff is empty - nothing to verify")
    ui.stage_ok("intent", f'"{intent_result.intent.splitlines()[0][:70]}"')
    ...
    if not kept:
        _fail("validate", "no scenario is traceable to this diff - nothing trustworthy to run")
    ui.stage_ok("validate", f"{len(kept)}/{len(validations)} traceable to the diff")
    for v in dropped:
        ui.console.print(f"      [red]✗[/] [dim strike]{v.scenario.name}[/] [red dim]{v.reason[:70]}[/]")
    ...
    if not tests:
        _fail("sandbox", "no scenario produced runnable test code - verdict is UNVERIFIED")
    ...
    run_id = new_run_id()
    record = build_record(run_id, intent_result, generation, results, risk, config.model)
    if ungeneratable:
