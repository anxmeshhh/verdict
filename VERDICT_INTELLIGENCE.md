...
3. ✅ UI: `/intelligence` on the existing FastAPI server (`verdict/server/api.py`) - risk ranking by repo, full finding list with correlation/status, read-only, live-verified with real data via FastAPI's TestClient (200 OK, correct rendering).
4. ✅ Setup: `verdict db init` already provisions the `findings` table (same `SCHEMA` constant as everything else) - no new command needed. `verdict health` now reports a `intelligence` line (open/alerted finding counts), verified live.

**Remediation-advisor and Verification-requester agents are not built yet** - Correlator + Triage were prioritized first since they're what the core "wow" moment (noticing a repeat vulnerability, autonomously) actually depends on. Both remaining agents follow the same wiring pattern already proven above.
