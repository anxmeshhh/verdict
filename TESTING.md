...
- [x] **[P1]** **`verdict init` with no flags, on a never-configured repo, now interactively asks local vs. cloud (2026-07-04)** instead of silently defaulting to ollama - Enter/`local` keeps the original default-ollama behavior unchanged; `cloud` prompts for provider + API key, then fetches the REAL model list (same `_list_models`/`_pick_from_list` picker `verdict model` already uses) instead of hard-failing on a missing `--model`. Flag-based invocation (`--provider ...`) is untouched - the prompt only fires when `provider` is `None` AND the repo has no `.verdict/config.json` yet; re-running bare `init` on an already-initialized repo does not re-prompt. Verified all three paths live: bare+Enter → local default (unchanged); bare+cloud+bad key → graceful "couldn't fetch the model list" error, no crash; `--provider`+flags → prompt skipped entirely, identical to before.
...
- [x] **[P1]** **New-user onboarding, ahead of Phase 6's real external users (2026-07-04).** Found by walking through the actual first-run experience: a brand-new, never-`init`'d repo showed `ollama down` in the shell banner with zero explanation (a fresh `Config()` defaults to ollama, which the user may never have wanted), and `help`/`--help` dumped ~20 commands as one flat, undifferentiated list. Fixed: (1) `verdict --help`'s top-level text now explains what the tool does and names the two commands that cover most use (`init` then `check`); (2) every command is grouped into "Getting started" / "Everyday" / "Admin, server & CI" panels via Typer's `rich_help_panel`, both at the top level and in the interactive shell's `help`; (3) a brand-new repo now shows a distinct first-run welcome (`ui.first_run_banner`) pointing straight at `init`, instead of the misleading liveness-check banner; (4) `init`'s success output now ends with a concrete next step (`verdict check`). Verified: a truly fresh `git init` repo shows the welcome banner, not `ollama down`; an already-`init`'d repo (the demo repo) shows the exact same banner as before - no regression.
...
- [x] **[P1]** **Hook block message now distinguishes exit 1 (risky) from exit 2 (couldn't verify) (2026-07-04).** Previously both printed the same "verification did not come back LOW" wording, even though exit 2 means the checker itself failed (bad ref, provider down), not that the code looked risky. Fixed: the hook captures `verdict run`'s actual exit status and prints "could not verify this push (a checker problem, not necessarily your code)" for exit 2, keeping the original wording for exit 1. Verified end-to-end with a real bare remote + a genuinely unreachable provider (invalid API key): the push was blocked with the new, correct message; `--no-verify` bypass re-confirmed unaffected.
...
## 17. Release prep (2026-07-04) - real distribution bug caught by a genuine stranger-install simulation

- [x] **[P0]** **PyPI name collision.** `verdict` is already taken on PyPI (an unrelated LLM-eval framework, Haize Labs). Renamed the distribution to `verdict-cli` in `pyproject.toml`; the CLI command itself stays `verdict` (`[project.scripts]` untouched) - only `pip install <name>` changes. Confirmed nothing else in the repo (Dockerfile, action.yml, setup scripts) hardcodes a PyPI package-name install - they all install from a local path, unaffected by the rename.
- [x] **[P0]** **Regression: `click` was an undeclared dependency, and a fresh install crashed on the first command.** `verdict/cli.py` imports `click` directly (`click.ClickException`, `click.exceptions.Exit`) but `pyproject.toml` never listed it - this worked in the dev environment purely because an old `click` happened to already be installed from months of earlier development, masking the bug completely. Caught by actually simulating a stranger: fresh venv, `pip install git+https://github.com/anxmeshhh/verdict.git` exactly as the new README instructs → `verdict --help` → `ModuleNotFoundError: No module named 'click'`. Root cause confirmed via `pip show typer` in the clean venv: newer Typer (0.26.8) no longer lists `click` among its own dependencies at all (`Requires: annotated-doc, colorama, rich, shellingham`) - it used to be a reliable transitive dependency and silently stopped being one. Fixed by adding `click>=8.0` as an explicit direct dependency. Re-verified clean: fresh venv, `verdict --help` works.
- [x] **[P1]** Audited every top-level import across `verdict/` and `verdict/server/` against declared dependencies - the only other cross-package case (`pydantic`, used directly in `server/api.py` but not separately declared) confirmed still safe via a real `pip install --dry-run --report` of `fastapi`: pydantic is a load-bearing, core FastAPI dependency (unlike click's relationship to typer), reliably pulled in every time.
- [x] **[P0]** Added a real `## Install` section to the README - previously nonexistent; the workflow diagram's `git clone <friend's project>` step is cloning the project being verified, not explaining how to get Verdict itself onto a machine. Also added standard release metadata (`readme`, `license`, `authors`, `keywords`, `classifiers`, `project.urls`) to `pyproject.toml`.
- Real PyPI publish itself is intentionally NOT done here - that's an external, semi-irreversible action requiring the user's own PyPI account/token. Today's install path (`pip install git+https://github.com/anxmeshhh/verdict.git`) is fully verified working end-to-end from a clean environment; publishing to PyPI as `verdict-cli` is a deliberate next step, not yet taken.
